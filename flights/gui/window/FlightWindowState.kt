...
class FlightWindowState(
    private val application: FlightApplicationState,
    path: Path?,
    private val exit: (FlightWindowState) -> Unit
) {
    ...
    private var _notifications = Channel<FlightWindowNotification>(0)
    val notifications: Flow<FlightWindowNotification> get() = _notifications.receiveAsFlow()
    ...
    private suspend fun save(path: Path) {
        ...
        try {
            saveJob?.join()
            _notifications.trySend(FlightWindowNotification.SaveSuccess(path))
        } catch (e: Exception) {
            isChanged = true
            e.printStackTrace()
            _notifications.trySend(FlightWindowNotification.SaveError(path))
        }
    }
...
sealed class FlightWindowNotification {
    class SaveSuccess(val path: Path) : FlightWindowNotification()
    class SaveError(val path: Path) : FlightWindowNotification()
}
