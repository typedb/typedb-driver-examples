...
@Composable
fun FlightWindow(state: FlightWindowState) {
...
private fun titleOf(state: FlightWindowState): String {
    val changeMark = if (state.isChanged) "*" else ""
    val filePath = state.path ?: "Untitled"
    return "$changeMark$filePath - Flight"
}

@Composable
private fun WindowNotifications(state: FlightWindowState) {
    // Usually we take into account something like LocalLocale.current here
    fun FlightWindowNotification.format() = when (this) {
        is FlightWindowNotification.SaveSuccess -> Notification(
            "File is saved", path.toString(), Notification.Type.Info
        )
        is FlightWindowNotification.SaveError -> Notification(
...
@Composable
private fun FrameWindowScope.WindowMenuBar(state: FlightWindowState) = MenuBar {
    val scope = rememberCoroutineScope()
