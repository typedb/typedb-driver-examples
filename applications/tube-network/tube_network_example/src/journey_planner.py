...
if __name__ == "__main__":
    ...
    with client.session(keyspace=settings.keyspace) as session:
        with session.transaction(grakn.TxType.READ) as transaction:
            ...
            a_id = get_match_id(list(transaction.query(match_get("$s1 isa station, has name \"{}\";".format(a_name)))), "s1")
            b_id = get_match_id(list(transaction.query(match_get("$s1 isa station, has name \"{}\";".format(b_name)))), "s1")
            ...
            shortest_paths = list(transaction.query(compute_query))
            print("...done")
            ...
            for i, unique_path in enumerate(unique_paths):
                ...
                for station_id in unique_path:
                    station_name = list(transaction.query("match $s1 id {}, has name $n; get $n;".format(station_id)))[0].get('n').value()
                    print("+ " + station_name)
