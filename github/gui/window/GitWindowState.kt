...
class GitWindowState(
    private val application: GitApplicationState,
    path: Path?,
    private val exit: (GitWindowState) -> Unit
) {
    ...
    private var _notifications = Channel<GitWindowNotification>(0)
    val notifications: Flow<GitWindowNotification> get() = _notifications.receiveAsFlow()
    ...
    private suspend fun save(path: Path) {
        ...
        try {
            saveJob?.join()
            _notifications.trySend(GitWindowNotification.SaveSuccess(path))
        } catch (e: Exception) {
            isChanged = true
            e.printStackTrace()
            _notifications.trySend(GitWindowNotification.SaveError(path))
        }
    }
...
sealed class GitWindowNotification {
    class SaveSuccess(val path: Path) : GitWindowNotification()
    class SaveError(val path: Path) : GitWindowNotification()
}
