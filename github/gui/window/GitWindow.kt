...
@Composable
fun GitWindow(state: GitWindowState) {
...
private fun titleOf(state: GitWindowState): String {
    val changeMark = if (state.isChanged) "*" else ""
    val filePath = state.path ?: "Untitled"
    return "$changeMark$filePath - Git"
}

@Composable
private fun WindowNotifications(state: GitWindowState) {
    // Usually we take into account something like LocalLocale.current here
    fun GitWindowNotification.format() = when (this) {
        is GitWindowNotification.SaveSuccess -> Notification(
            "File is saved", path.toString(), Notification.Type.Info
        )
        is GitWindowNotification.SaveError -> Notification(
...
@Composable
private fun FrameWindowScope.WindowMenuBar(state: GitWindowState) = MenuBar {
    val scope = rememberCoroutineScope()
